...
public class FrmAtualizar extends Formulario {
	...
	private void salvar() {
		if (SSMensagem.pergunta("Confirma atualizar este lançamento ?")) {
			...
			if(SSMensagem.pergunta("Lançamento atualizado com sucesso!\nDeseja compensar?")) {
				lanctoDao.compensarLancamento(entidade.getId(), txtDataPagamento.getDataHora());
				SSMensagem.informa("Compensação realizada com sucesso");
			}
			super.fechar();
		}
	}
