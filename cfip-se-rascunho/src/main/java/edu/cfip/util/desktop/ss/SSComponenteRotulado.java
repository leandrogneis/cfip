...
import edu.cfip.util.desktop.ss.evento.ValidacaoEvento;
import edu.cfip.util.desktop.ss.evento.ValidacaoListener;
...
public abstract class SSComponenteRotulado extends JPanel implements SSComponente {
    ...
    private PosicaoRotulo rotuloPosicao;
    ...
    private void jbInit() throws Exception {
        ...
        setRotuloPosicao(PosicaoRotulo.TOPO);
    }
    
    public void setRotuloPosicao(PosicaoRotulo rotuloPosicao) {
    ...
    public PosicaoRotulo getRotuloPosicao() {
    ...
    public synchronized void addValidacaoListener(ValidacaoListener pl) {        
        listenerList.add(ValidacaoListener.class, pl);
    }

    public synchronized void removeValidacaoListener(ValidacaoListener pl) {        
        listenerList.remove(ValidacaoListener.class, pl);
    }
    
    public synchronized ValidacaoListener[] getValidacaoListeners() {        
        return listenerList.getListeners(ValidacaoListener.class);
    }
    ...
    private boolean componente_verify(JComponent input) {
        ...
        ValidacaoListener[] listeners = getValidacaoListeners(); 
        ValidacaoEvento evento;
        //Dispara ação
        for (ValidacaoListener listener : listeners) {
            evento = new ValidacaoEvento(this);
