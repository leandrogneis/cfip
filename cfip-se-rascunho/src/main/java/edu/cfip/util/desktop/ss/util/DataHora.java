...
package edu.cfip.util.desktop.ss.util;
...
public  class DataHora {
    ...
    public static java.util.Date ultimoDiaDoMes() {
    	return ultimoDiaDoMes(new Date());
    }
    public static java.util.Date ultimoDiaDoMes(java.util.Date data) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(data);
        calendar.set(DIA, calendar.getActualMaximum(Calendar.DAY_OF_MONTH));
        return calendar.getTime();
    }
    public static java.util.Date primeiroDiaDoMes() {
    	return primeiroDiaDoMes(new Date());
    }
    public static java.util.Date primeiroDiaDoMes(java.util.Date data) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(data);
    ...
    public static java.util.Date horaMinima(java.util.Date data) {        
        data = DataHora.definir(data, DataHora.HORA, 0);
        data = DataHora.definir(data, DataHora.MINUTO, 0);
        data = DataHora.definir(data, DataHora.SEGUNDO, 0);
        return data;
    }
    
    public static java.util.Date horaMaxima(java.util.Date data) {        
        data = DataHora.definir(data, DataHora.HORA, 23);
        data = DataHora.definir(data, DataHora.MINUTO, 59);
        data = DataHora.definir(data, DataHora.SEGUNDO, 59);
        return data;
    }
    
    public static int converterPeriodo(String periodo) {        
        periodo = Texto.substitui(periodo, "/", "");
        if (periodo.length() > 4)
            periodo = periodo.substring(2) + periodo.substring(0, 2);
        return Integer.parseInt(periodo);
    }
   
    public static Date copiarHora(java.util.Date origem, java.util.Date destino) {
    ...
    private static int pega(java.util.Date dataHora, int item) {
    ...
    public static Integer pegaPeriodo(Date dataHora) {
        String periodo = pegaAno(dataHora) + "" + Formatador.formatar(pegaMes(dataHora), "00");
        return Integer.parseInt(periodo);
    }
    
    public static int pegaAno(java.util.Date dataHora) {
        return pega(dataHora, ANO);
    }
    
    public static int pegaMes(java.util.Date dataHora) {
        return pega(dataHora, MES) + 1;
    }
    
    public static int pegaDia(java.util.Date dataHora) {
    ...
    public static Date adiciona(int intervalo, int numero, java.util.Date data){
    ...
    public static Date definir(java.util.Date data, int intervalo, int numero){
        GregorianCalendar gc = new GregorianCalendar();
        gc.setTime(data);
    ...
    public static String nomeCompletoData(Date data) {
        String dia = Formatador.formatar(data, "dd");
